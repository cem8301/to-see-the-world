...
class CoordinatesToCountries:
    ...
    def get_geodata(self, coords):
        ...
        missed_ids = [start_ids.index(x) for x in start_ids
            if x not in complete_ids]
        if len(missed_ids) == 0:
            return df1
        print(f'num missed_ids: {len(missed_ids)}, '
            'running kdtree')
        m_ids = [coords['id'][idx] for idx in missed_ids]
        m_coords = [coords['coords'][idx] for idx in missed_ids]
        missed_coords = {'id': m_ids, 'coords': m_coords}
        df2 = self.get_geodata_kdtree(missed_coords)
        df_combined = pd.concat([df1, df2], ignore_index=True)
        return df_combined
    
    def clockwise_sort_polygon(self, coords):
        centroids = self.Datasets.get_centroid(coords)
        center_x = centroids[1]
        center_y = centroids[0]
        def angle_to_center(coord):
            return (math.atan2(coord[1] - center_y, coord[0
                ] - center_x) + 2 * math.pi) % (2 * math.pi)
        ans = sorted(coords, key=angle_to_center, reverse=True)
        polygon = Polygon(ans)
    ...
    def get_polygons(self):
        df_grouped = self.df_cb_shifted.groupby('fid').apply(
            lambda x: list(zip(x['lat'], x['lon']))
            ).reset_index(name='tuples')
        df_grouped['tup_len'] = df_grouped.tuples.apply(len)
        df_grouped = df_grouped.get(df_grouped.tup_len >= 4
            ).reset_index()
        a = self.df_cb_shifted.set_index('fid')['country_code'
            ].to_dict()
        idx_to_fid = df_grouped.to_dict()['fid']
        cc_index_dict = {k:a[v] for k,v in idx_to_fid.items()}
    ...
    def get_geodata_strtree(self, coords):
        ...
        polygons, cc_index_dict, idx_to_fid = self.get_polygons() 
        points = [Point(v) for v in coords['coords']]
        tree = STRtree(points)
        cgi = tree.query(polygons, predicate='contains')
        geo_data = {
            'id': [], 'fid': [], 'country_code': [], 'og_coord': []}
        coords_dict = {idx:{
            'id': coords['id'][idx],
        ...
        for k,v in c.items():
            geo_data['country_code'].append(cc_index_dict[v])
            geo_data['id'].append(coords_dict[k]['id'])
            geo_data['fid'].append(idx_to_fid[v])
            geo_data['og_coord'].append(coords_dict[k]['tuple'])
    ...
    def get_geodata_kdtree(self, coords):
        ...
        _, ii = tree.query(coords['coords'], k=1, workers=-1)
        geo_data = {
            'id': [], 'fid': [], 'country_code': [], 'og_coord': []}
        for idx, i in enumerate(ii):
            ...
            geo_data['og_coord'].append(coords['coords'][idx])
        return pd.DataFrame(geo_data)
